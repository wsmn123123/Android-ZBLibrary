	...
	EndScene = function(c) {
		...
		d.prototype.showOff = function(b) {
			...
			if( /liebao/gi.test(navigator.userAgent) ){
				...
				tipimg.src = 'http://dl.tx024.com/hotdog/share2.png';
...
var poisonCount=0;
var _url = "http://dl.tx024.com/hotdog/?t=" + (new Date()).valueOf();;
var __extends = this.__extends || function(c, d) {
		function b() {
			this.constructor = c
		}
		...
		d.prototype.share = function() {
			...
			function setShare(title, content){
				LBGAME.Share.updateData({
					...
					title:title||"一起来玩【王校长吃热狗】！超好玩！",
					imgUrl:"http://dl.tx024.com/hotdog/icon.png",
					cont:content||"我看你和王校长的差距主要在吃热狗的方式，来挑战一下吧！",
					url:_url,
					doneJump:"http://www.zzfriend.com/h5game/",
...
var __extends = this.__extends || function(c, d) {
	...
	GameScene = function(c) {
		...
		d.prototype.share = function() {
			var title = "我挑战#王校长吃热狗#，跑了"+ this.bestDistance + "00 m，躲开了"+this.endScene.percent.text+"个炸弹，你能超过我吗?";
			setShare(title, title);
			LBGAME.Share.updateData({
				call:true,// false只更新数据||true更新数据+调启用户端分享
				title:"我挑战#王校长吃热狗#，跑了"+ this.bestDistance + "00 m，吃掉了"+this.endScene.percent.text+"个炸弹，你能超过我吗?",
				imgUrl:"http://dl.tx024.com/hotdog/icon.png",
				cont:"我挑战#王校长吃热狗#，跑了"+ this.bestDistance + "00 m，吃掉了"+this.endScene.percent.text+"个炸弹，你能超过我吗?",
				url:_url,
				doneJump:"http://www.zzfriend.com/h5game/",
				fun:"" // 分享时回调，不 fun:function(){}
			});
